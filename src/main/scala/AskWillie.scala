...
@main def main(): Unit = {
        ...
        // Load WebPage.id -> WebPage map to better handle graph // 1
        ...
        val rankedPages: List[RankedWebPage] = PageRank.pagerank(pages).map((k, v) => new RankedWebPage(pages(k), v)).toList // call PageRank.???? here
        ...
        while {
          ...
          val searchedPages: List[SearchedWebPage] = PageSearch.count(rankedPages, terms).zip(rankedPages).toMap.map((k, v) => new SearchedWebPage(v, k)).toList // call PageSearch.???? here
